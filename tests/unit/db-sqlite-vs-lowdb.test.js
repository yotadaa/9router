...
beforeAll(async () => {
  ...
  sqliteAdapter = await getAdapter();
});
...
describe("DB SQLite layer — public API parity", () => {
  ...
  it("proxyPools: bulk create persists the same defaults", async () => {
    const created = await sqliteDb.bulkCreateProxyPools([
      { name: "bulk-1", proxyUrl: "http://bulk-one.test:8080" },
      { name: "bulk-2", proxyUrl: "socks5://bulk-two.test:1080", isActive: false },
      { name: "bulk-3", proxyUrl: "http://bulk-three.test:3128", strictProxy: true },
    ]);

    expect(created).toBe(3);

    const pools = await sqliteDb.getProxyPools();
    const imported = pools.filter((pool) => pool.name.startsWith("bulk-"));
    expect(imported).toHaveLength(3);
    expect(imported.find((pool) => pool.name === "bulk-1")).toMatchObject({
      type: "http",
      isActive: true,
      strictProxy: false,
      testStatus: "unknown",
    });
    expect(imported.find((pool) => pool.name === "bulk-2")?.isActive).toBe(false);
    expect(imported.find((pool) => pool.name === "bulk-3")?.strictProxy).toBe(true);

    for (const pool of imported) {
      await sqliteDb.deleteProxyPool(pool.id);
    }

    await expect(sqliteDb.bulkCreateProxyPools([
      { name: "rollback-good", proxyUrl: "http://rollback-good.test:8080" },
      { name: "rollback-bad", proxyUrl: 1n },
    ])).rejects.toThrow();

    const afterRollback = await sqliteDb.getProxyPools();
    expect(afterRollback.some((pool) => pool.name === "rollback-good")).toBe(false);
  });
