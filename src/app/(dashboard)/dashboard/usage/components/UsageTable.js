...
import { fmt, fmtCost2 as fmtCost } from "@/shared/utils/format";
 ...
 */
export function ValueCells({ item, viewMode, isSummary = false }) {
  if (viewMode === "tokens") {
    return (
      <>
 ...
 *   Receives (item, { expandable, expanded }) — render a chevron when expandable is true.
 * @param {function} props.renderSummaryCells - Render summary row cells after group label (placeholder cols)
 * @param {function} [props.renderItemSubRows] - Optional sub-rows rendered under an expanded detail row
 *   (e.g. per-API-key rows under a model's provider row). Receives the item, returns rows.
 * @param {function} [props.renderExpandedExtra] - Optional extra rows rendered when a group is expanded
 *   (e.g. per-API-key breakdown under a model group). Receives the group, returns rows.
            ...
            {groupedData.map((group) => (
              <Fragment key={group.groupKey}>
                ...
                {expanded.has(group.groupKey) && group.items.map((item) => {
                  const childKey = `${group.groupKey}::${item.key}`;
                  const hasSubRows = !!renderItemSubRows;
                  const subExpanded = expanded.has(childKey);
                  return (
                    <Fragment key={`detail-${item.key}`}>
                      <tr
                        className={`group-detail hover:bg-bg-subtle/20 transition-colors ${hasSubRows ? "cursor-pointer" : ""}`}
                        onClick={hasSubRows ? () => toggleGroup(childKey) : undefined}
                      >
                        {renderDetailCells(item, { expandable: hasSubRows, expanded: subExpanded })}
                        <ValueCells item={item} viewMode={viewMode} />
                      </tr>
                      {hasSubRows && subExpanded && renderItemSubRows(item)}
                    </Fragment>
                  );
                })}
                {/* Extra expanded content under a group (optional) */}
                {expanded.has(group.groupKey) && renderExpandedExtra?.(group)}
              </Fragment>
            ))}
...
UsageTable.propTypes = {
  ...
  renderExpandedExtra: PropTypes.func,
