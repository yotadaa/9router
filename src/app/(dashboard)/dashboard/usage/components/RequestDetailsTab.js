...
import { flattenContent } from "./chatVisualUtils";
...
export default function RequestDetailsTab() {
  ...
  const [detailViewMode, setDetailViewMode] = useState("chat");
      ...
      >
        {selectedDetail && (
          <div className="space-y-6">
            ...
            {/* View mode toggle: Chat visual vs Raw JSON */}
            <div className="flex items-center justify-between gap-2 border-t border-black/5 pt-4 dark:border-white/5">
              <SegmentedControl
                options={[
            ...
            {detailViewMode === "chat" && <ChatVisual detail={selectedDetail} />}

            <div className={cn("space-y-4", detailViewMode !== "raw" && "hidden")}>
              ...
              <CollapsibleSection title="4. Client Response (Final)" defaultOpen={true} icon="output">
                {(() => {
                  // response.content/thinking may be a string OR Claude-format
                  // content blocks ({type, thinking|text} objects/arrays) — flatten before rendering.
                  const thinkingText = selectedDetail.response?.thinking
                    ? flattenContent(selectedDetail.response.thinking)
                    : "";
                  const contentText = selectedDetail.response?.content
                    ? flattenContent(selectedDetail.response.content)
                    : "";
                  return (
                    <>
                      {thinkingText && (
