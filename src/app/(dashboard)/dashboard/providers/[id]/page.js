...
import { Card, Button, Badge, Input, Modal, CardSkeleton, OAuthModal, KiroOAuthWrapper, CursorAuthModal, IFlowCookieModal, GitLabAuthModal, Toggle, Select, EditConnectionModal, NoAuthProxyCard, ConfirmModal, PriceConfigModal } from "@/shared/components";
...
import ProxyPoolPicker from "./ProxyPoolPicker";
...
export default function ProviderDetailPage() {
  ...
  const [selectedPriceModelId, setSelectedPriceModelId] = useState(null);
  ...
  const fetchConnections = useCallback(async () => {
    try {
      const [connectionsRes, nodesRes, proxyPoolsRes, settingsRes] = await Promise.all([
        ...
        fetch("/api/proxy-pools?page=1&pageSize=100&isActive=true&testStatus=active&sort=latency-asc", { cache: "no-store" }),
  ...
  const handlePriceConfigSaved = async () => {
    // Recompute historical costs with the new pricing so /dashboard/usage
    // reflects the configured rates (costs are normally fixed at request time).
    fetch("/api/usage/recalculate-costs", { method: "POST" }).catch(() => {});
    await fetchConnections();
  };
  ...
  const rememberProxyPool = (pool) => {
    if (!pool?.id) return;
    setProxyPools((current) => (
      current.some((candidate) => candidate.id === pool.id)
        ? current.map((candidate) => candidate.id === pool.id ? { ...candidate, ...pool } : candidate)
        : [...current, pool]
    ));
  };

  const handleApplySinglePool = async (pool) => {
    const proxyPoolId = pool?.id || null;
    setBulkProxyPoolId(proxyPoolId || "__none__");
    const targets = connections.map((c) => ({ connectionId: c.id, proxyPoolId }));
    await applyProxyAssignments(targets);
    rememberProxyPool(pool);
  };

  const handleApplyOneToOne = () => {
    if (proxyPools.length === 0) {
      alert("No health-valid proxy pools available.");
      return;
    }
          ...
          <div key={conn.id} className="flex min-w-0 items-stretch">
            ...
            <div className="flex-1 min-w-0">
              <ConnectionRow
                ...
                onUpdateProxy={async (proxyPoolId, selectedPool) => {
                  try {
                    ...
                    if (res.ok) {
                      rememberProxyPool(selectedPool);
  ...
  const bulkActionModal = (
    <Modal
      ...
      title={`Proxy for connections (${connections.length})`}
    >
      <div className="flex flex-col gap-3">
        <div className="flex flex-col">
          <button
            onClick={handleApplyOneToOne}
            disabled={bulkUpdatingProxy || proxyPools.length === 0}
          ...
          >
            ...
            <span className="text-sm text-text-main">Rotate across fastest valid proxies</span>
          </button>
          <div className="overflow-hidden rounded-lg border border-border">
            <ProxyPoolPicker
              value={bulkProxyPoolId === "__none__" ? null : bulkProxyPoolId}
              onSelect={handleApplySinglePool}
              disabled={bulkUpdatingProxy}
              enabled={showBulkProxyModal}
              noneLabel="None (unbind all connections)"
            />
          </div>
        </div>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        {customModelRows.map((model) => (
          <ModelRow
            ...
            onConfigurePricing={() => {
              setSelectedPriceModelId(model.id);
