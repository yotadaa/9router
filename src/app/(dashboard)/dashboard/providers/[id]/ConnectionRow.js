...
export default function ConnectionRow({ connection, proxyPools, isOAuth, isFirst, isLast, onMoveUp, onMoveDown, onToggleActive, onUpdateProxy, onEdit, onDelete, oneByOneStatus = null, autoPing = null }) {
  ...
  const handleSelectProxy = async (pool) => {
    setUpdatingProxy(true);
    try {
      await onUpdateProxy(pool?.id || null, pool || null);
    } finally {
        ...
        <div className="grid flex-1 grid-cols-3 gap-1 sm:flex sm:flex-none">
          ...
          {((proxyPools || []).length > 0 || hasAnyProxy) && (
            <div className="relative" ref={proxyDropdownRef}>
              ...
              {showProxyDropdown && (
                <div className="absolute right-0 top-full z-50 mt-1 w-[min(24rem,calc(100vw-2rem))] overflow-hidden rounded-lg border border-border bg-bg shadow-lg">
                  <ProxyPoolPicker
                    value={boundProxyPoolId}
