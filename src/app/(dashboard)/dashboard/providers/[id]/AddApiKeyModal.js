...
export default function AddApiKeyModal({ isOpen, provider, providerName, isCompatible, isAnthropic, authType, authHint, website, proxyPools, error, existingNames, onSave, onBulkDone, onClose }) {
  ...
  const buildProviderSpecificData = () => {
    ...
    if (isXaiApiKey && formData.customBaseUrl.trim()) {
      return { customBaseUrl: formData.customBaseUrl.trim() };
    }
    if (isAzure) {
      return {
    ...
    <Modal isOpen={isOpen} title={`Add ${providerName || provider} ${credentialLabel}`} onClose={onClose}>
      <div className="flex flex-col gap-4">
        ...
        {isXaiApiKey && (
          ...
          />
          <p className="text-xs text-text-muted">
            Optional OpenAI-compatible base URL for Text to Image and Web Search (e.g. <code>https://ha.ntu.my.id/v1/</code>). Leave empty to use <code>api.x.ai</code>.
          </p>
          </>
        )}
        {isCookie && authHint && (
