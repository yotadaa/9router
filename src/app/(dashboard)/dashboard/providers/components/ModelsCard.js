...
import { Card, Button, Modal, PriceConfigModal } from "@/shared/components";
import { getModelsByProviderId, getModelKind } from "@/shared/constants/models";
import { getProviderAlias } from "@/shared/constants/providers";
import { useCopyToClipboard } from "@/shared/hooks/useCopyToClipboard";

// ── ModelRow ───────────────────────────────────────────────────
export function ModelRow({ model, fullModel, copied, onCopy, testStatus, isCustom, isFree, onDeleteAlias, onTest, isTesting, onConfigurePricing }) {
  ...
  return (
    <div className={`group px-3 py-2 rounded-lg border ${borderColor} hover:bg-sidebar/50`}>
      <div className="flex items-center gap-2">
        ...
        {onConfigurePricing && (
          <div className="relative group/btn">
            <button onClick={onConfigurePricing} className="p-0.5 hover:bg-blue-500/10 rounded text-text-muted hover:text-blue-500 transition-opacity ml-1">
...
export default function ModelsCard({ providerId, kindFilter, providerAliasOverride }) {
  ...
  const handleConfigurePricing = (modelId) => {
  ...
  return (
    <>
          ...
          {displayModels.map((model) => {
            ...
            return (
              <ModelRow
                ...
                isFree={model.isFree}
                onConfigurePricing={() => handleConfigurePricing(model.id)}
              />
            );
          })}
      ...
      <PriceConfigModal
        ...
        providerId={providerAliasOverride || getProviderAlias(providerId)}
