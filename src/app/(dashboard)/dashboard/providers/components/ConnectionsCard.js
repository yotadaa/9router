...
const PROXY_POOL_PICKER_URL = "/api/proxy-pools?view=picker&page=1&pageSize=100&sort=latency-asc";
...
function ConnectionRow({ connection, proxyPools, isOAuth, isFirst, isLast, onMoveUp, onMoveDown, onToggleActive, onUpdateProxy, onEdit, onDelete }) {
  ...
  const proxyDisplayText = boundProxyPool
    ...
    : boundProxyPoolId ? `Pool: ${boundProxyPoolId} (outside fastest valid set)`
  ...
  const proxyBadgeVariant = boundProxyPool?.isActive === true ? "success" : "default";
        ...
        <div className="flex flex-wrap gap-1">
          {((proxyPools || []).length > 0 || hasAnyProxy) && (
            <div className="relative" ref={proxyDropdownRef}>
              ...
              {showProxyDropdown && (
                <div className="absolute right-0 top-full mt-1 z-50 max-h-64 min-w-[220px] max-w-[min(90vw,24rem)] overflow-y-auto bg-bg border border-border rounded-lg shadow-lg py-1">
                  ...
                  {boundProxyPoolId && !boundProxyPool && (
                    <button disabled className="w-full truncate px-3 py-1.5 text-left text-sm font-medium text-primary/70" title={boundProxyPoolId}>
                      Current: {boundProxyPoolId}
                    </button>
                  )}
                  {(proxyPools || []).map((pool) => (
                    <button key={pool.id} onClick={() => handleSelectProxy(pool.id)} title={pool.name} className={`w-full truncate px-3 py-1.5 text-left text-sm hover:bg-black/5 dark:hover:bg-white/5 ${boundProxyPoolId === pool.id ? "text-primary font-medium" : "text-text-main"}`}>{pool.name}</button>
                  ))}
                </div>
              )}
            </div>
...
function AddApiKeyModal({ isOpen, provider, providerName, proxyPools, onSave, onClose }) {
  ...
  const [formData, setFormData] = useState({ name: "", apiKey: "", priority: 1, proxyPoolId: NONE, customBaseUrl: "" });
  ...
  const handleValidate = async () => {
    ...
    try {
      const res = await fetch("/api/providers/validate", {
        ...
        body: JSON.stringify({ provider, apiKey: formData.apiKey, ...(isXai && formData.customBaseUrl.trim() ? { providerSpecificData: { customBaseUrl: formData.customBaseUrl.trim() } } : {}) }),
      });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const providerSpecificData = isXai && formData.customBaseUrl.trim()
  ...
  return (
    <Modal isOpen={isOpen} title={`Add ${providerName || provider} API Key`} onClose={onClose}>
      <div className="flex flex-col gap-4">
        ...
        {isXai && (
          <div>
            <label className="text-xs text-text-muted mb-1 block">Custom Host</label>
            <input className="w-full px-3 py-2 text-sm border border-border rounded-lg bg-background focus:outline-none focus:border-primary" value={formData.customBaseUrl} onChange={(e) => setFormData({ ...formData, customBaseUrl: e.target.value })} placeholder="https://api.x.ai/v1/" />
            <p className="text-xs text-text-muted mt-1">Optional OpenAI-compatible base URL for Text to Image and Web Search. Leave empty to use api.x.ai.</p>
          </div>
        )}
        <div>
        ...
        <Select label="Proxy Pool" value={formData.proxyPoolId} onChange={(e) => setFormData({ ...formData, proxyPoolId: e.target.value })}
          hint="Showing up to 100 fastest health-valid pools."
  ...
  const fetch_ = useCallback(async () => {
    try {
      const [connRes, proxyRes, settingsRes] = await Promise.all([
        ...
        fetch(PROXY_POOL_PICKER_URL, { cache: "no-store" }),
