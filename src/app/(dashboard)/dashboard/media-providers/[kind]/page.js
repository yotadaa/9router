...
import { Card, Badge, Button, Toggle, AddCustomEmbeddingModal, AddCustomImageModal, AddCustomVideoModal } from "@/shared/components";
...
// Kinds that support user-defined custom provider nodes (providerNodes table)
const KIND_NODE_TYPES = { embedding: "custom-embedding", image: "custom-image", video: "custom-video" };
const CUSTOM_NODE_META = {
  "custom-embedding": { label: "Custom Embedding", color: "#6366F1", textIcon: "CE" },
  "custom-image": { label: "Custom Image", color: "#8B5CF6", textIcon: "CI" },
  "custom-video": { label: "Custom Video", color: "#EC4899", textIcon: "CV" },
};
...
export default function MediaProviderKindPage() {
  ...
  const customNodeType = KIND_NODE_TYPES[kind] || null;
  ...
  useEffect(() => {
    ...
    if (customNodeType) {
      fetch("/api/provider-nodes", { cache: "no-store" })
        .then((r) => r.json())
        .then((d) => setCustomNodes((d.nodes || []).filter((n) => n.type === customNodeType)))
        .catch(() => {});
    }
    if (supportsCombo) {
  ...
  }, [customNodeType, supportsCombo, kindConfig]);
  ...
  const customProviders = customNodes.map((n) => {
    const meta = CUSTOM_NODE_META[n.type] || { label: "Custom", color: "#8B5CF6", textIcon: "C" };
    return { id: n.id, name: n.name || meta.label, color: meta.color, textIcon: meta.textIcon };
  });
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {customNodeType === "custom-video" && (
        <AddCustomVideoModal
          isOpen={showAddCustomVideo}
          onClose={() => setShowAddCustomVideo(false)}
          onCreated={(node) => {
            setCustomNodes((prev) => [...prev, node]);
