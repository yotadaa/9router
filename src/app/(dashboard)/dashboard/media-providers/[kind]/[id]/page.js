...
import { Card, Badge, Button, AddCustomEmbeddingModal, AddCustomImageModal, AddCustomVideoModal, NoAuthProxyCard, ProviderInfoCard } from "@/shared/components";
import ProviderIcon from "@/shared/components/ProviderIcon";
import { MEDIA_PROVIDER_KINDS, AI_PROVIDERS, isCustomEmbeddingProvider, isCustomImageProvider, isCustomVideoProvider } from "@/shared/constants/providers";
...
// Custom provider node meta (badge color/icon + display kind)
const CUSTOM_NODE_META = {
  "custom-embedding": { label: "Custom Embedding", kind: "embedding", color: "#6366F1", textIcon: "CE" },
  "custom-image": { label: "Custom Image", kind: "image", color: "#8B5CF6", textIcon: "CI" },
  "custom-video": { label: "Custom Video", kind: "video", color: "#EC4899", textIcon: "CV" },
};
...
export default function MediaProviderDetailPage() {
  ...
  const customKind = (isCustomEmbeddingProvider(id) && kind === "embedding")
    ? "custom-embedding"
    : (isCustomImageProvider(id) && kind === "image")
      ? "custom-image"
      : (isCustomVideoProvider(id) && kind === "video")
        ? "custom-video"
        : null;
  const isCustom = !!customKind;

  const handleDeleteCustom = async () => {
    if (!confirm(`Delete this ${CUSTOM_NODE_META[customKind]?.label || "custom"} provider?`)) return;
    try {
  ...
  // For custom provider nodes, build a synthetic provider object
  const customMeta = CUSTOM_NODE_META[customKind];
  const provider = isCustom
    ? (customNode ? { id, name: customNode.name || customMeta.label, color: customMeta.color, textIcon: customMeta.textIcon } : null)
    : builtInProvider;
  ...
  const kinds = isCustom ? [customMeta.kind] : (provider.serviceKinds ?? ["llm"]);
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {(kind === "image" || kind === "video") && KIND_EXAMPLE_CONFIG[kind] && (
        <GenericExampleCard providerId={id} kind={kind} customAlias={isCustom ? customNode?.prefix : undefined} />
      )}
      {!isCustom && kind !== "image" && kind !== "video" && KIND_EXAMPLE_CONFIG[kind] && <GenericExampleCard providerId={id} kind={kind} />}

      {isCustom && customKind === "custom-embedding" && (
      ...
      {isCustom && customKind === "custom-image" && (
        <AddCustomImageModal
          isOpen={showEditModal}
          node={customNode}
          onClose={() => setShowEditModal(false)}
      ...
      {isCustom && customKind === "custom-video" && (
        <AddCustomVideoModal
          isOpen={showEditModal}
          node={customNode}
          onClose={() => setShowEditModal(false)}
