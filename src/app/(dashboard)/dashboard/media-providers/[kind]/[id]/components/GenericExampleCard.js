...
export function GenericExampleCard({ providerId, kind, customAlias }) {
  const providerAlias = customAlias || getProviderAlias(providerId);
  const resolvedId = resolveProviderId(providerAlias);
  // For custom provider nodes, requests use the node prefix (customAlias)
  const safeProviderAlias = customAlias
    ? providerAlias
  ...
  // Get models for this kind (e.g., type="image"), merged with user-added custom models
  const [customModels, setCustomModels] = useState([]);
  const kindModels = useMemo(() => {
    const registry = getModelsByProviderId(providerId).filter((m) => getModelKind(m) === kind);
    return [...registry, ...customModels.filter((m) => !registry.some((r) => r.id === m.id))];
  }, [providerId, kind, customModels]);
  ...
  // Load user-added custom models for this provider/kind (added via the Models card)
  const loadCustomModels = useCallback(() => {
    fetch("/api/models/custom", { cache: "no-store" })
      .then((r) => r.json())
      .then((d) => {
        setCustomModels((d.models || []).filter((m) => getModelKind(m) === kind && m.providerAlias === providerAlias));
      })
      .catch(() => {});
  }, [kind, providerAlias]);
  ...
  useEffect(() => {
    ...
    fetch("/api/providers/client")
      ...
      .catch(() => {});
    loadCustomModels();
  }, [providerId, loadCustomModels]);

  // Refresh the model list when a custom model is added/deleted in the Models card
  useEffect(() => {
    window.addEventListener("customModelChanged", loadCustomModels);
    return () => window.removeEventListener("customModelChanged", loadCustomModels);
  }, [loadCustomModels]);

  // Custom models arrive async — pick the first model if the selection is empty/stale
  useEffect(() => {
    if (needsModel && kindModels.length > 0 && !kindModels.some((m) => m.id === selectedModel)) {
      setSelectedModel(kindModels[0].id);
    }
  }, [needsModel, kindModels, selectedModel]);
