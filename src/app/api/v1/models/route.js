...
function providerMatchesKinds(providerId, kindFilter) {
  // Custom provider nodes are not in the registry — match by node type
  if (typeof providerId === "string" && providerId.startsWith("custom-image-")) return kindFilter.includes("image");
  if (typeof providerId === "string" && providerId.startsWith("custom-embedding-")) return kindFilter.includes("embedding");
  if (typeof providerId === "string" && providerId.startsWith("custom-video-")) return kindFilter.includes("video");
...
export async function buildModelsList(kindFilter, options = {}) {
  ...
  if (connections.length === 0) {
    ...
    for (const customModel of customModels) {
      ...
      // Honor the requested kind (custom models default to llm when untyped)
