...
import { isOpenAICompatibleProvider, isAnthropicCompatibleProvider, isCustomImageProvider } from "@/shared/constants/providers";
...
export async function GET(request, { params }) {
  try {
    ...
    if (isOpenAICompatibleProvider(connection.provider) || isCustomImageProvider(connection.provider)) {
      let baseUrl = connection.providerSpecificData?.baseUrl;
      if (!baseUrl) {
        return NextResponse.json({ error: "No base URL configured for OpenAI compatible provider" }, { status: 400 });
      }
      // Custom-image nodes may store a full images endpoint; normalize to the API base
      baseUrl = baseUrl.replace(/\/$/, "");
      if (baseUrl.endsWith("/images/generations")) {
        baseUrl = baseUrl.slice(0, -"/images/generations".length);
      }
