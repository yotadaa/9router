...
async function testApiKeyConnection(connection, effectiveProxy = null) {
  ...
  try {
    switch (connection.provider) {
      ...
      case "xai": {
        // Custom host connections validate against the self-hosted gateway instead of api.x.ai
        const modelsUrl = customBaseUrlEndpoint("https://api.x.ai/v1/models", "/models", connection);
        const res = await fetchWithConnectionProxy(modelsUrl, { headers: { Authorization: `Bearer ${connection.apiKey}` } }, effectiveProxy);
        return { valid: res.ok, error: res.ok ? null : "Invalid API key" };
      }
      case "nvidia": {
