...
import { customBaseUrlEndpoint } from "open-sse/handlers/imageProviders/_base.js";
    ...
    try {
      ...
      switch (provider) {
        ...
        case "nvidia": {
          ...
          // xAI custom host: validate the key against the self-hosted gateway instead of api.x.ai
          const validateTarget = provider === "xai"
            ? customBaseUrlEndpoint(endpoints[provider], "/models", { providerSpecificData })
            : endpoints[provider];
          const usesXaiCustomHost = validateTarget !== endpoints[provider];
          const res = await fetch(validateTarget, { headers, signal: AbortSignal.timeout(8000) });
          // xai returns 400 for bad key, 403 for valid-but-no-credit. Other providers use 401.
          // A custom gateway may not mirror x.ai's /models quirks — accept anything but 401 there.
          if (provider === "xai") {
            isValid = usesXaiCustomHost ? res.status !== 401 : (res.status === 200 || res.status === 403);
