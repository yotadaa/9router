...
import { USAGE_APIKEY_PROVIDERS, APIKEY_USAGE_ENABLED_PROVIDERS } from "@/shared/constants/providers";
...
export async function GET(request, { params }) {
  ...
  try {
    ...
    // Allow OAuth connections, plus whitelisted apikey providers (glm/minimax/kiro/qoder/...)
    ...
    // NEW: Also allow API key connections for providers that support quota tracking with API keys
    const hasApikeyUsage =
      isApikeyAuth && APIKEY_USAGE_ENABLED_PROVIDERS.includes(connection.provider);

    if (!isOAuth && !isApikeyEligible && !hasApikeyUsage) {
