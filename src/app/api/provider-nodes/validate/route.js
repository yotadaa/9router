...
export async function POST(request) {
  try {
    ...
    // Custom Image Validation - cheap GET /models probe (validates the key without
    // spending a generation; image capability is proven at first real request)
    if (type === "custom-image") {
      let normalizedBase = baseUrl.trim().replace(/\/$/, "");
      if (normalizedBase.endsWith("/images/generations")) {
        normalizedBase = normalizedBase.slice(0, -"/images/generations".length);
      }
      const res = await fetchWithTimeout(`${normalizedBase}/models`, {
    ...
    // Custom Video Validation - cheap GET /models probe against the API base
    // (the video base is {api}/videos; models live one level up at {api}/models)
    if (type === "custom-video") {
      let normalizedBase = baseUrl.trim().replace(/\/$/, "");
      if (normalizedBase.endsWith("/videos")) {
        normalizedBase = normalizedBase.slice(0, -"/videos".length);
      }
      const res = await fetchWithTimeout(`${normalizedBase}/models`, {
        headers: { "Authorization": `Bearer ${apiKey}` },
      });
      ...
      // Some video gateways expose only /videos (no /models) — not an auth failure
      if (res.status === 404) {
        return NextResponse.json({ valid: true, method: "models", warning: "No /models endpoint — key not verified" });
      }
      return NextResponse.json({ valid: false, error: getModelsErrorMessage(res.status), method: "models" });
    }
