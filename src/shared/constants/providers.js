...
// Providers that support API key auth AND quota tracking (for dynamic eligibility)
// These providers can use either OAuth or API key/PAT for usage fetching
const APIKEY_USAGE_ENABLED_PROVIDERS = REGISTRY
  .filter(r => r.features?.usage && r.authModes?.includes("apikey"))
  .map(r => r.id);

// Expose for use in isUsageEligible filter
export { APIKEY_USAGE_ENABLED_PROVIDERS };
