...
import UsageTable, { ValueCells, fmt, fmtTime } from "@/app/(dashboard)/dashboard/usage/components/UsageTable";
...
const MODEL_COLUMNS = [
  ...
  { field: "providerKey", label: "Provider Key" },
  ...
  const activeTableConfig = useMemo(() => {
    ...
    switch (tableView) {
      case "model": {
        ...
        // Index per-provider-account rows by (rawModel | provider) so each
        // model's provider row expands into the PROVIDER credentials/accounts
        // (connectionId-based) that actually carried the requests upstream.
        // byAccount entries carry the same display provider name as byModel.
        const accountRows = sortData(stats.byAccount, {}, "requests", "desc");
        const keysByModelProvider = {};
        for (const row of accountRows) {
          const mk = `${row.rawModel || ""}|${row.provider || ""}`;
          (keysByModelProvider[mk] ||= []).push(row);
        }
        return {
          ...
          renderDetailCells: (item, { expandable, expanded }) => {
            const keys = keysByModelProvider[`${item.rawModel || ""}|${item.provider || ""}`] || [];
            return (
              <>
                <td className={`px-6 py-3 font-medium transition-colors ${item.pending > 0 ? "text-primary" : ""}`}>
                  <span className="inline-flex items-center gap-1.5">
                    {expandable && keys.length > 0 && (
                      <span className={`material-symbols-outlined text-[14px] text-text-muted transition-transform ${expanded ? "rotate-90" : ""}`}>
                        chevron_right
                      </span>
                    )}
                    {item.rawModel}
                ...
                <td className="px-6 py-3 text-text-muted text-xs">
                  {keys.length > 0 ? `${keys.length} provider key${keys.length === 1 ? "" : "s"}` : "—"}
                </td>
          ...
          renderItemSubRows: (item) => {
            const keys = keysByModelProvider[`${item.rawModel || ""}|${item.provider || ""}`] || [];
            return keys.map((k) => (
              <tr key={`provkey-${k.key}`} className="group-detail bg-bg-subtle/10 hover:bg-bg-subtle/20 transition-colors">
                <td className="px-6 py-2"></td>
                <td className="px-6 py-2"></td>
                <td className="px-6 py-2">
                  <span className="inline-flex items-center gap-1.5 text-xs">
                    <span className="material-symbols-outlined text-[14px] text-text-muted">vpn_key</span>
                    <span className="font-medium">{k.accountName || `Account ${k.connectionId?.slice(0, 8)}...`}</span>
                    {k.connectionId && (
                      <code className="rounded bg-black/5 px-1 py-0.5 font-mono text-[10px] text-text-muted dark:bg-white/10">
                        {k.connectionId.slice(0, 8)}
                      </code>
                    )}
                  </span>
                </td>
                <td className="px-6 py-2 text-right text-xs">{fmt(k.requests)}</td>
                <td className="px-6 py-2 text-right text-text-muted text-xs whitespace-nowrap">{fmtTime(k.lastUsed)}</td>
                <ValueCells item={k} viewMode={viewMode} />
              </tr>
            ));
          },
        };
      ...
      <div className="flex flex-col gap-3">
        ...
        {loading ? spinner : activeTableConfig && (
          <UsageTable
            ...
            renderItemSubRows={activeTableConfig.renderItemSubRows}
