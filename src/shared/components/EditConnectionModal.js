...
export default function EditConnectionModal({ isOpen, connection, proxyPools, onSave, onClose }) {
  ...
  useEffect(() => {
    if (connection) {
      setFormData({
        ...
        apiKey: "",
        customBaseUrl: connection.providerSpecificData?.customBaseUrl || "",
      });
  ...
  const isXai = connection?.provider === "xai";
    ...
    try {
      const res = await fetch("/api/providers/validate", {
        ...
        body: JSON.stringify({
          ...
          ...(isXai && formData.customBaseUrl.trim() ? { providerSpecificData: { customBaseUrl: formData.customBaseUrl.trim() } } : {}),
        }),
      });
      const data = await res.json();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Custom host for xAI Text to Image + Web Search (empty value clears the override)
      if (isXai) {
        const customBaseUrl = formData.customBaseUrl.trim();
        updates.providerSpecificData = {
          ...(updates.providerSpecificData || {}),
          ...(customBaseUrl ? { customBaseUrl } : { customBaseUrl: null }),
        };
      }
      ...
      <div className="flex flex-col gap-4">
        ...
        {isXai && (
          <Input
            ...
            hint="Optional OpenAI-compatible base URL for Text to Image and Web Search. Leave empty to use api.x.ai."
          />
        )}
