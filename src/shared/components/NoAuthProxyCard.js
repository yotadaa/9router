...
export default function NoAuthProxyCard({ providerId }) {
  ...
  const [activeProxyPoolCount, setActiveProxyPoolCount] = useState(0);
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all([
      fetch(PROXY_POOL_PICKER_URL, { cache: "no-store" }).then((r) => r.ok ? r.json() : { proxyPools: [] }),
      fetch("/api/settings", { cache: "no-store" }).then((r) => r.ok ? r.json() : {}),
    ]).then(([poolData, settingsData]) => {
      if (cancelled) return;
      setProxyPools(poolData.proxyPools || []);
      setActiveProxyPoolCount(
        Number.isFinite(poolData.pagination?.active)
          ? poolData.pagination.active
          : (poolData.proxyPools || []).length
      );
  ...
  const canRotate = activeProxyPoolCount >= 2;
  const isRotation = rotateStrategy !== "none";
  const hasCurrentPoolCandidate = proxyPoolId === NONE_PROXY_POOL_VALUE
    || proxyPools.some((pool) => pool.id === proxyPoolId);
    ...
    <Card>
      ...
      <Select
        ...
        options={[
          ...
          ...(!hasCurrentPoolCandidate
            ? [{ value: proxyPoolId, label: `Current pool (${proxyPoolId})` }]
            : []),
          ...proxyPools.map((pool) => ({ value: pool.id, label: pool.name })),
        ]}
        hint={isRotation
          ? "Pool selector is ignored when rotation is active — all active pools are used."
          : "Showing up to 100 fastest health-valid pools."}
      />
          ...
          {!canRotate
            ...
            : isRotation
              ? rotateStrategy === "round-robin"
                ? `Rotating through all ${activeProxyPoolCount} active pools in order. State is in-memory (resets on restart).`
                : `Picking a random pool from ${activeProxyPoolCount} active pools each request.`
