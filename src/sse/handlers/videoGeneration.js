...
// User-defined custom video provider nodes (open-sse has no registry entry for
// them — videoCore rebuilds their config from the connection snapshot).
...
async function resolveVideoProvider(parsedBody) {
  ...
  const isCustomVideoNode = typeof modelInfo.provider === "string" && modelInfo.provider.startsWith(CUSTOM_VIDEO_PREFIX);
  if (!isCustomVideoNode && !getVideoConfig(modelInfo.provider)) {
...
export async function handleVideoGet(request, requestId) {
  ...
  // Jobs are account-bound upstream: when the caller pins the creating
  // connection, derive the provider from it — this is what makes custom video
  // nodes pollable (their baseUrl/shape live on the connection, not xai).
