...
/**
 * Recalculate costs for all usageHistory rows using the CURRENT pricing config,
 * then rebuild the usageDaily aggregates from history. Use this after the user
 * adds/changes model pricing so past requests pick up the configured rates
 * (costs are normally computed at request time and historical rows stay stale).
 *
 * Note: the daily byAccount/byApiKey/byEndpoint maps are rebuilt from history
 * rows only; connectionId→name and apiKey metadata come from the rows themselves
 * (aggregateEntryToDay keys by connectionId/apiKey, not display name).
 */
export async function recalculateUsageCosts() {
  ...
  for (const r of rows) {
    ...
    if (r.provider && r.model) {
      ...
      if (pricing) cost = calculateCostFromTokens(tokens, pricing);
    }
    updated.push({ id: r.id, cost });

    const entry = {
      timestamp: r.timestamp, provider: r.provider, model: r.model,
      connectionId: r.connectionId, apiKey: r.apiKey, endpoint: r.endpoint,
      cost, status: r.status, tokens,
    };
...
/**
 * Compact per-day rows for the activity calendar / overview page.
 * Tokens are pre-summed server-side; heavy sub-maps (byAccount/byApiKey/
 * byEndpoint) are omitted. maxDays: null/undefined → all rows.
 * Returns [{ dateKey, requests, tokens, cost, byProvider, byModel }] asc.
 */
export async function getUsageDaily(maxDays = null) {
  const db = await getAdapter();
  const rows = loadDaysInRange(db, maxDays);
...
export async function getUsageStats(period = "all") {
  ...
  if (useDailySummary) {
    const periodDays = { "7d": 7, "30d": 30, "60d": 60, "12mo": 365 };
...
export async function getChartData(period = "7d") {
  ...
  // 12mo aggregates daily rows into 12 monthly buckets (keeps the chart
  // light — 12 points instead of ~365 daily points).
  if (period === "12mo") {
    const monthCount = 12;
    const buckets = [];
    const bucketByMonth = {}; // "YYYY-MM" -> bucket index
    for (let i = 0; i < monthCount; i++) {
      const d = new Date(today.getFullYear(), today.getMonth() - (monthCount - 1 - i), 1);
      const ym = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`;
      bucketByMonth[ym] = buckets.length;
      buckets.push({
    ...
    for (const r of loadDaysInRange(db, 365)) {
      const idx = bucketByMonth[r.dateKey.slice(0, 7)]; // "YYYY-MM-DD" → "YYYY-MM"
      if (idx === undefined) continue;
      const day = parseJson(r.data, {});
      buckets[idx].tokens += (day.promptTokens || 0) + (day.completionTokens || 0);
      buckets[idx].cost += day.cost || 0;
    }
    return buckets;
  }
