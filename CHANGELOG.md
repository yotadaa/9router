...
## Features
- **Providers**: custom host for xAI — per-connection `customBaseUrl` (OpenAI-compatible base URL, e.g. a self-hosted gateway) routes Text to Image (`/v1/images/generations`) and Web Search (`/v1/responses`) through it; settable in the add/edit connection dialogs, empty falls back to `api.x.ai`
- **Media Providers**: custom Text to Image providers — add your own OpenAI-compatible image endpoint (provider nodes, like custom embedding) on `/dashboard/media-providers/image`: pick a compatible-provider preset (derived from the registry) or any base URL, check the key against `{baseUrl}/models`, then add/import models (`{prefix}/{model}` request format); connection keys, multi-key fallback, `/v1/models` listing and model test all supported
- **Media Providers**: custom Video providers on `/dashboard/media-providers/video` — add any OpenAI-compatible video job gateway as a provider node; choose the job API shape (xAI-style `POST {base}/generations` or OpenAI Sora-style `POST {base}`, both poll `GET {base}/{jobId}`); job creation, polling and account-bound polls via `x-connection-id` all route through the node's base URL
- **Media Providers**: custom models (added on the Models card) now appear in the example/test card instantly
