...
import { resolveCustomBaseUrl, RESPONSES_PATH } from "../imageProviders/_base.js";
...
const CHAT_SEARCH_CONFIG = {
  ...
  xai: {
    // Honors per-connection customBaseUrl (e.g. self-hosted OpenAI-compatible gateway)
    endpoint: (model, creds) => resolveCustomBaseUrl(searchEndpoint("xai", model), RESPONSES_PATH, creds),
    buildBody: (query, model) => ({
      model,
